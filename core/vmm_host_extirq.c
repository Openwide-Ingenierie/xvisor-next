...
static int _extirq_expand(void)
{
	...
	if (!irqs) {
		pr_err("%s: Failed to reallocate extended IRQ array from "
	...
	if (!bitmap) {
		pr_err("%s: Failed to reallocate extended IRQ bitmap from "
		       "%d to %d bytes\n", __func__, old_size, new_size);
		vmm_free(irqs);
		return VMM_ENOMEM;
	}
...
static int extirq_find_free_region(unsigned int			size)
{
	...
	if (pos < 0) {
		pr_err("%s: Failed to find an extended IRQ region\n",
		       __func__);
		return pos;
	}
...
vmm_host_extirq_group_get(unsigned int	irq_num)
{
	...
	vmm_mutex_unlock(&extirqctrl.lock);
	pr_err("%s: Failed to find IRQ %d group\n", __func__, irq_num);
				   ...
				   unsigned int			irq_num)
{
	...
	if (NULL == (irq = vmm_malloc(sizeof (struct vmm_host_irq)))) {
		pr_err("%s: Failed to allocate IRQ\n", __func__);
		return VMM_ENOMEM;
	}

	name_len = strlen(group->of_node->name) + 6;
	host_irq_init_irq(irq, group->base + irq_num);

	name = vmm_malloc(name_len);
	if (!name) {
		pr_err("%s: Failed to allocate IRQ name\n", __func__);
		vmm_free(irq);
		return VMM_ENOMEM;
	}
		    ...
		    void					*host_data)
{
	...
	if (0 > (pos = extirq_find_free_region(size))) {
		pr_err("%s: Failed to find available slot for IRQ\n",
		       __func__);
		return NULL;
	}
