...
static int pwm_backlight_parse_dt(struct vmm_device *dev,
				  struct platform_pwm_backlight_data *data)
{
	...
	if (data->max_brightness > 0) {
		...
		data->levels = vmm_devm_zalloc(dev, size);
		if (!data->levels)
			return -ENOMEM;
			       ...
			       const struct vmm_devtree_nodeid *nodeid)
{
	...
	pb = vmm_devm_zalloc(dev, sizeof(*pb));
	if (!pb) {
		ret = -ENOMEM;
		goto err_alloc;
	}
