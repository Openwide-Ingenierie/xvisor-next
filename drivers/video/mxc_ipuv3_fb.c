...
static int mxcfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	vmm_printf("%d(+%d %d) * %d(+%d %d) @%d * %d (actual pixclock %d - "
		   "0x%x)\n",
		   var->xres, var->right_margin, var->left_margin, var->yres,
		   var->upper_margin, var->lower_margin, var->hsync_len,
		   var->vsync_len, var->pixclock, &var);
...
static struct fb_info *mxcfb_init_fbinfo(struct vmm_device *dev, struct fb_ops *ops)
{
	...
	vmm_printf("PIXCLK at init: %d (0x%x)\n", fbi->var.activate,
		   &fbi->var);
