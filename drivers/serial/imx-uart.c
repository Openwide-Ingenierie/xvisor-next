 ...
 * to allow a full UART initialization without the need of a bootloader, and to
 * correct and allow using the UART TX interruption, with a menuconfig option.
 *
...
static u32 imx_read(struct vmm_chardev *cdev, u8 * dest, size_t len,
...
static u32 imx_write(struct vmm_chardev *cdev, u8 * src, size_t len,
		     off_t __unused *off, bool sleep)
{
	u32 i;
	struct imx_port *port;
